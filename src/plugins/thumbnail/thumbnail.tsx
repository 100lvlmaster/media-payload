...
const Thumbnail: React.FC<Props> = (props) => {
  ...
  const classes = [baseClass, `${baseClass}--size-${size || 'medium'}`].join(
    ' '
  );
  ...
  if (thumbnail) {
    switch (thumbnail.type) {
      case 'image':
        thumbnailComponent = (
          <img src={thumbnail?.src} alt={filename as string} />
        );
        break;
      case 'video':
        thumbnailComponent = (
  ...
  return <div className={classes}>{thumbnailComponent}</div>;
};
